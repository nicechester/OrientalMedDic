"""
Donut Fine-tuning: 동의보감 한자 인식 모델 학습
"""
import json
...
DATASET_DIR = Path(__file__).parent.parent / "data" / "donut_dataset_combined"
OUTPUT_DIR = Path(__file__).parent.parent / "models" / "donut-hanjadic-v4"
...
class HanjaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ground truth 텍스트를 토큰화
        gt = json.loads(entry["ground_truth"])
        parse = gt.get("gt_parse", gt)
        text = TASK_PROMPT + json.dumps(parse, ensure_ascii=False) + self.processor.tokenizer.eos_token
        labels = self.processor.tokenizer(
            text, max_length=self.max_length, padding="max_length", truncation=True, return_tensors="pt"
        ).input_ids.squeeze()
        # pad token은 loss에서 무시
...
def main():
    ...
    # 태스크 프롬프트 토큰 추가
    special_tokens = {"additional_special_tokens": [TASK_PROMPT]}
    ...
    # 데이터셋
    train_ds = HanjaDataset(DATASET_DIR / "train", processor)
    val_ds = HanjaDataset(DATASET_DIR / "validation", processor)
